...
function getOwnerOfWallet(web3, address, abi, callback) {
    ...
    try {
        BetaWalletContract.methods._owner().call((error, res) => {
            if (!error) {
                callback(res, 0);   
            }
            else {
...
function transferToToken(web3, address, abi, beneficiary, amount, type,  callback) {
    ...
    const BetaWalletContract =  new web3.eth.Contract(abi, address, {from: web3.eth.defaultAccount});
            ...
            try {
                BetaWalletContract.methods.transferToToken(beneficiary, amount, type).send((error, res) => {
                    if (!error) {
                        callback(res, 0);   
                    }
                    else {
...
function signTransaction(web3, address, abi,transactionId, callback) {
    ...
    const BetaWalletContract =  new web3.eth.Contract(abi, address);
            try {
                BetaWalletContract.methods.signTransaction(transactionId).send((error, res) => {
                    if (!error) {
                         callback(res, 0);   
                    }
                    else {
...
function deletePendingTransaction(web3, address, abi,transactionId, callback) {
    ...
    const BetaWalletContract =  new web3.eth.Contract(abi, address);
                try {
                    BetaWalletContract.methods.deletePendingTransaction(transactionId).send((error, res) => {
                     if (!error) {
                            callback(res, 0);   
                     }
                        else {
...
function setNewAdmin(web3, address, abi, newAdmin, callback) {
    ...
    const BetaWalletContract =  new web3.eth.Contract(abi, address);
                try {
                    BetaWalletContract.methods.setNewAdmin(newAdmin).send((error, res) => {
                     if (!error) {
                            callback(res, 0);   
                     }
                        else {
...
function withdraw_ether(web3, address, abi, callback) {
    ...
    const BetaWalletContract =  new web3.eth.Contract(abi, address);
                try {
                    BetaWalletContract.methods.withdraw_ether((error, res) => {
                        if (!error) {
                             callback(res, 0);   
                        }
                         else {
...
function withdraw_token(web3, address, abi, token_amount, callback) {
    ...
    const BetaWalletContract =  new web3.eth.Contract(abi, address);
                try {
                    BetaWalletContract.methods.withdraw_token(token_amount),send((error, res) => {
                        if (!error) {
                             callback(res, 0);   
                          }
                         else {
