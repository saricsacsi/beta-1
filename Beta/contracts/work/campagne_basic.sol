...
contract Campagne_basic is Ownable {

    BetaToken  public token;
    event Send(address indexed beneficiary, uint256 indexed amount);
    ...
    function send(address[] dests, uint256[] values)
    onlyOwner
    public
    returns (uint256) {
        uint256 i = 0;
